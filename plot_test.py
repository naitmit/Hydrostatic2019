"""
Edited version of plot_bot used for zooming in on the Pycnocline and plotting
bouyancy contours.
Only the plot_bot function has been edited. Added integration routine for calculating
bouyancy.
Must hard-code the contour plotting: I plotted contours near the peak of N^2.
Need to find initial bouyancy at the peak of N^2 and specify the interval in which to plot
contours - hence the integration.
This still requires some eyeballing to find the appropriate amount of contours, due to
the nonlinearity of N^2
"""
...
def integ(x=-H/40): #finds bouyancy contour value at specified height
    z2 = np.arange(x, 0 , 1e-8) #integration domain
    return simps(fill_N2(z2),z2)
'----------------------------------------------'
...
def plot_bot(dset, image_axes, data_slices, k, image_scales=(0,0), clim=None, even_scale=False, cmap='RdBu_r', axes=None, figkw={}, title=None, func=None):
    """
    ...
    k: int
        k=1, contour is plotted and the hardcoded ylim is set.
        This is not done is k is not set to any other value
    ...
    # contour plot
    if k==1:
        levels=np.linspace(integ(-H/20+H/40), integ(-H/20-H/40),25) #have 25 contours spaced in the given interval
        paxes.contour(xmesh, ymesh, data, levels, colors='k',
                               linewidths=0.50, linestyles='-')
    '---------------------------------------'
    ...
    '------------------Zoom-----------------------'
    if k == 1:
        paxes.set_ylim(-H/20-H/40, -H/20 + H/40) #set zoom
    '---------------------------------------------'
    return paxes, caxes
